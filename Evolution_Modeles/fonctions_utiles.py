    ...
    for building in liste_nodes_buildings:
        ...
        if len(liste_intersection_points)>0 :
            if pt in [b.center for b in liste_intersection_points[:nb_roads_intersections]]:
                pt = (pt[0] + 10**-PRECISION, pt[1] + 10**-PRECISION)
...
class Graph:
    ...
    def connected_buildings(self, verbose=False):
        ...
        return connected, count, network_length, network_nodes_idx, heat_demand_total

def tracer_graphe_optimal(commune, reseaux_communes) :
    #Tracé le graphe optimal trouvé
    G, network_nodes, connected, liste_nodes_buildings, filtered_lines = reseaux_communes[commune]["graph"],reseaux_communes[commune]["network_nodes"], reseaux_communes[commune]["connected_buildings"], reseaux_communes[commune]["liste_nodes_buildings"], reseaux_communes[commune]["filtered_lines"]
...
def tracer_graphe_optimal_zoom(commune, reseaux_communes) : 
    G, network_nodes, connected, liste_nodes_buildings, filtered_lines = reseaux_communes[commune]["graph"], \
    reseaux_communes[commune]["network_nodes"], \
    reseaux_communes[commune]["connected_buildings"], \
    reseaux_communes[commune]["liste_nodes_buildings"], \
    reseaux_communes[commune]["filtered_lines"]
