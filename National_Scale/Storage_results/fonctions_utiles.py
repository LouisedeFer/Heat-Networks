...
def selection_geom_batiments_routes(df_buildings, df_roads) :
    ...
    df_buildings["distance_centre"] = (
        df_buildings["centroids"]
        .dropna()
    ...
    index_highest_central_consumer = df_10_centre['conso_ref_tot_MWh'].idxmax()
    ...
    return df_buildings,filtered_lines, index_highest_central_consumer
...
class Graph:
    ...
    def used_network_nodes(self, connected_buildings): #pour tracer le graphe optimal
        used = set()
        stack = list(connected_buildings)
        ...
        return {
            ip for ip in self.liste_intersection_points
            if ip.index in used
        }
...
def tracer_graphe_optimal(commune, result) :
    ...
    #afficher le plant en rouge
    plant=G.ip_plant
    plt.scatter(plant.coordinates[0][0], plant.coordinates[0][1], color='red', s=40, label='Plant')
    # afficher le batiment d'index 0 en orange
    building_0 = liste_nodes_buildings[0].coordinates[0]
    plt.scatter(building_0[0], building_0[1], color='orange', s=40, label='Bâtiment d\'index 0')
    # afficher le batiment d'index 1 en magenta
    building_1 = liste_nodes_buildings[1].coordinates[0]
    plt.scatter(building_1[0], building_1[1], color='magenta', s=40, label='Bâtiment d\'index 1')
...
def process_commune(
    ...
    df_buildings_sorted_final, filtered_lines, index_highest_central_consumer = selection_geom_batiments_routes(
    ...
    if len(connected) < NB_MIN: #on retente avec un autre bâtiment de départ
        ...
        G=Graph(
            ...
            liste_intersection_points,
            voisins,
            index_highest_central_consumer
        )
        G.dijkstra()
        connected, network_length, network_nodes, heat_demand = (
        G.connected_buildings(verbose=False)
        ...
        if len(connected) < NB_MIN:
            print(f"Commune {commune} définitivement rejetée au stade 3 : {len(connected)} bâtiments connectés et bâtiment de départ : {index_highest_central_consumer}")
            return CommuneOutcome(
                "rejected", commune, "3",
                f"Stade 3 : {len(connected)} bâtiments connectés, densité {heat_demand/network_length if network_length>0 else 0:.2f}",
                len(connected), len(liste_nodes_buildings), None
            )
        
        print(f"La commune {commune} a finalement été sélectionnée en prenant {index_highest_central_consumer} comme bâtiment de départ, avec {len(connected)} bâtiments.")

    if network_length <= 0:
        print(f"Commune {commune} rejetée au stade 3 (réseau vide ou longueur nulle) : {network_length} ")
        return CommuneOutcome(
        "rejected", commune, "3",
        "Réseau vide ou longueur nulle",
        len(connected), len(liste_nodes_buildings), None
    )
